...
function analyzeLines(text, strippedText, referenceConstants, tokensByLine) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (openBraces > closeBraces) {
            ...
            if (firstTok && firstTok.type === TokenType.IDENTIFIER && ['table', 'grid', 'tablegraph', 'gridgraph', 'vectordraw'].includes(firstTok.value.toLowerCase())) {
...
function findEquationLabelRegions(line, lineTokens) {
    ...
    if (lbrace) {
        ...
        const isTable = firstTok && firstTok.type === TokenType.IDENTIFIER && ['table', 'grid', 'tablegraph', 'gridgraph', 'vectordraw'].includes(firstTok.value.toLowerCase());
        if (!isTable) {
