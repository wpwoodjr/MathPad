...
- **`vectorDraw("Title"; type[; fontSize]) = { body }`** — vector diagram with start/end pairs. `type` is required: `navigation` (0° = up, +° clockwise — bearings), `polar` (0° = right, +° counter-clockwise — math), or `cartesian` (raw x/y, no angle handling). For navigation/polar each pair is (direction, magnitude); for cartesian each pair is (x, y). Optional `fontSize` is the third argument.
