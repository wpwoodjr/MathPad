...
- **Under-determined detection**: the post-recursion classifier reports `"Too many unknowns"` for any equation with ≥2 remaining unknowns — including bare-LHS definitions like `x = a + b` with `b` unknown (symmetric with `a + b = x`). `isSkippableDefEquation` (used mid-recursion in Kind 2/Kind 3) skips bare-LHS definitions either when RHS is fully known (Kind 1 direct-eval will handle it) or when LHS is unbound (let substitutions propagate first) — keeps traces clean. The post-recursion classifier doesn't need a definition-equation filter: if RHS is fully known, direct-eval has already bound the LHS and `eqUnknowns < 2`; if RHS still has unknowns, the equation genuinely is under-determined and should surface.
