...
- **Body-def authority** (`context.firedBodyDefs`, `reportUnevaluableBodyDefs`): body defs (`:` declarations with a non-literal RHS) fire in phase [1] of each Advance pass when their RHS becomes evaluable. The solver isn't blocked from binding body-def-bound vars — direct-eval / Brent's may seed a value first; phase [1] then fires the body def and overrides the seeded value. Each def fires AT MOST ONCE per branch (apply-once via `context.firedBodyDefs`, snapshot/restored, automatically shared across components via the context). If a def never fires (RHS deps never bound, self-ref deadlock, etc.), `reportUnevaluableBodyDefs` emits a "Cannot evaluate" error — even when the solver bound the LHS to a value, so the body-def's failure isn't silent. **Known limitation**: solver-set vars derived from a value that a body def later overrides aren't invalidated, causing order-dependent staleness in patterns like `z: z+1` with `z = sqrt(x²+y²)` and a downstream `w = sqrt(z*z+...)`.
...
- **Balance loop owns equation-level error reporting**: after `solveRecursive` returns, the balance loop iterates equations per component and produces all equation-line errors. Categories and behavior:
  - **Unknowns** (any vars not in `context.variables`): emits `"Line N: Unknown in equation: X"` (1 unknown) or `"Line N: Too many unknowns (X, Y)"` (≥2). Sets `firstFailedEq` for highlighting.
  - **Balance fails** (left ≠ right within tolerance, or NaN/Infinity mismatch): emits `"Line N: Equation doesn't balance: ..."`. Sets `firstFailedEq`.
  - **Exception during evaluation** (`"Unknown function: X"`, `"Maximum call stack size exceeded"`, etc.): emits `"Line N: <message>"` for *every* affected equation. Sets `firstFailedEq` only on first.
  - **Focus rule** applies to balance/unknowns failures (one error per component), NOT to evaluation exceptions (which are independent structural issues — every `modClose` reference, every wrong-arity call, etc. gets reported).
  - **Parse errors** for equations missing a side (`= expr` or bare `=`) reported up front as `"Line N: Equation missing left side"`. `expr =` is the PalmOS-legacy evaluate-LHS pattern (handled separately).
- **Under-determined detection**: a definition equation `x = a + b` with `b` unknown reports "Unknown in equation" symmetrically with `a + b = x`. `isDefinitionEquation` recognizes a bare-identifier on *either* side (LHS-first when both sides are bare), so `def.variable` is the bare side and `def.expressionAST` is the other side regardless of writing direction. `isSkippableDefEquation` (mid-recursion in Kind 2/Kind 3) skips a definition when its expression side is fully known (Kind 1 direct-eval will handle it) or its variable side is unbound (let substitutions propagate first) — keeps traces clean.
- **Slow-path noise filter**: `resolveWithLimits` strips `"Unknown in equation"` / `"Too many unknowns"` messages from the slow-path's internal `solveEquations` result before propagating. Those describe the slow-path's local view (target var forcibly unbound to be re-solved) and would mislead the user about the actual record state. Only the primary slow-path failure (Brent's no-root, limit rejection, "no consistent value") surfaces.
- **UI highlight layer is independent of error severity**: variables-panel.js no longer suppresses green/orange/partial when hard errors exist. The data layer drives colors honestly — vars in `equationVarStatus` get their status; vars whose declaration line appears in any error message get `has-error` (red) layered on top via the line-number regex.
