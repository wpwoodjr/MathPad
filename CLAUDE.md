...
- **Under-determined detection**: the post-recursion classifier reports `"Too many unknowns"` for any equation with ≥2 remaining unknowns — including reversed-form definitions like `(a+b)/c = x` with `b` unknown (symmetric with `x = (a+b)/c`). `isDefinitionEquation` recognizes a bare-identifier on *either* side (LHS-first when both sides are bare), so `def.variable` is the bare side and `def.expressionAST` is the other side regardless of writing direction. `isSkippableDefEquation` (mid-recursion in Kind 2/Kind 3) skips a definition when its expression side is fully known (Kind 1 direct-eval will handle it) or its variable side is unbound (let substitutions propagate first) — keeps traces clean. The post-recursion classifier doesn't need a definition-equation filter: if the expression side is fully known, direct-eval has already bound the variable and `eqUnknowns < 2`; otherwise the equation genuinely is under-determined and should surface.
